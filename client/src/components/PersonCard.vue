...
export default {
  ...
  computed:{
    ...mapGetters(['getCurrentUserId', 'isLoggedIn', 'getFriends', 'getIfUserHasRequestFromUser', 'getIfUserHasSentRequestToUser']),
    ...
    ableToSendFriendRequest: function(){
      return this.isLoggedIn && this.user._id !== this.getCurrentUserId 
      && !this.getFriends.includes(this.user._id) && !this.getIfUserHasRequestFromUser(this.user._id) && !this.getIfUserHasSentRequestToUser(this.user._id) 
    }
  },
  methods: {
